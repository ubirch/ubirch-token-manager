...
class TokenApiSpec extends TestBase {
  ...
  "Token API" must {
    ...
    "external verification" in {
      ...
      assert(await(tokenApi.externalStateVerify(token, UUID.fromString("840b7e21-03e9-4de7-bb31-0b9524f3b500")), 3 seconds))
    ...
    "external verification sync" in {
      ...
      tokenApi.externalStateVerifySync(token, UUID.fromString("840b7e21-03e9-4de7-bb31-0b9524f3b500"))(3 seconds) match {
...
class FakeTokenManagerImpl extends TokenManagerImpl {
  ...
  override val tokenVerification: TokenVerification = new DefaultTokenVerification(config, tokenPublicKey, jsonConverter)
