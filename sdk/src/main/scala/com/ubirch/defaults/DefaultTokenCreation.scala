...
class DefaultTokenCreation @Inject() (jsonConverterService: JsonConverterService) extends TokenCreation with LazyLogging {
  ...
  override def encode(by: String, to: String, about: String, expiresIn: Option[Int], otherClaims: Content, privKey: PrivKey): Try[String] = {
    for {
      ...
      oc <- jsonConverterService.toString[Content](otherClaims).toTry
...
object DefaultTokenCreation {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def go(rawPrivKeyAsHex: String) = for {
      ...
      creation: TokenCreation = new DefaultTokenCreation(new DefaultJsonConverterService()(new JsonFormatsProvider() get() ))
